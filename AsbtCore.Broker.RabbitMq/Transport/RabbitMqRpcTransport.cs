...
namespace AsbtCore.Broker.RabbitMq.Transport
{
    public sealed class RabbitMqRpcTransport : IRpcTransport, IAsyncDisposable, IDisposable
    {
        ...
        public async Task<RpcResponse> SendAsync(RpcRequest request, string route, TimeSpan? timeout = null, CancellationToken cancellationToken = default)
        {
            ...
            // Единственная точка сериализации: object → byte[] прямо перед BasicPublish.
            ...
            try
            {
                await publishLock.WaitAsync(linkedCts.Token);
                try
                {
        ...
        private Task OnResponseReceivedAsync(object sender, BasicDeliverEventArgs ea)
        {
            try
            {
                ...
                // Единственная точка десериализации: ReadOnlyMemory<byte> → RpcResponse.
